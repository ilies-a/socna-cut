...
import styles1 from  './multirangeslider.module.css';
import styles2 from './multirangesliderblack.module.css';
...
const MultiRangeSlider = (props: Props, ref: React.ForwardedRef<HTMLDivElement>): JSX.Element => {
	...
	const wrapperClassName1 = props.baseClassName || 'multi-range-slider';
	const wrapperClassName2 = props.className || '';

	return (
		<div ref={ref} className={`${styles1[wrapperClassName1]} ${styles1[wrapperClassName2]}`} style={props.style} onWheel={onMouseWheel}>
			<div className={styles1['bar']} ref={refBar}>
				<div className={styles1['bar-left']} style={{ width: barMin + '%', backgroundColor: props.barLeftColor }} onClick={onBarLeftClick}></div>
				<input className={`${styles1['input-type-range']} ${styles1['input-type-range-min']}`} type='range' min={min} max={max} step={step} value={minValue} onInput={onInputMinChange} />
				<div className={`${styles1['thumb']} ${styles1['thumb-left']}`} style={{ backgroundColor: props.thumbLeftColor }} onMouseDown={onLeftThumbMousedown} onTouchStart={onLeftThumbTouchStart}>
					<div className={styles1['caption']}>
						<span className={styles1['min-caption']}>{minCaption}</span>
					</div>
				</div>
				<div className={styles1['bar-inner']} style={{ backgroundColor: props.barInnerColor }}>
					<div className={styles1['bar-inner-left']} onClick={onInnerBarLeftClick}></div>
					<div className={styles1['bar-inner-right']} onClick={onInnerBarRightClick}></div>
				</div>
				<input className={`${styles1['input-type-range']} ${styles1['input-type-range-max']}`} type='range' min={min} max={max} step={step} value={maxValue} onInput={onInputMaxChange} />
				<div className={`${styles1['thumb']} ${styles1['thumb-right']}`} style={{ backgroundColor: props.thumbRightColor }} onMouseDown={onRightThumbMousedown} onTouchStart={onRightThumbTouchStart}>
					<div className={styles1['caption']}>
						<span className={styles1['max-caption']}>{maxCaption}</span>
					</div>
				</div>
				<div className={styles1['bar-right']} style={{ width: barMax + '%', backgroundColor: props.barRightColor }} onClick={onBarRightClick}></div>
			</div>
			{ruler && (
				<div className={styles1['ruler']}>
					{[...Array(stepCount)].map((e, i) => (
						<div key={i} className={styles1['ruler-rule']}>
							{subSteps && [...Array(10)].map((e, n) => <div key={n} className={styles1['ruler-sub-rule']}></div>)}
						</div>
					))}
				</div>
			)}
			{label && (
				<div className={styles1['labels']}>
					{labels.map((label) => {
						return (
							<div key={label.toString()} className={styles1['label']}>
								{label}
							</div>
						);
					})}
